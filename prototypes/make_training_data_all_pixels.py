...
file_name_save = os.path.join('/home/vanvalen/DeepCell2/training_data_npz/', 'HeLa_set1_all_pixels_61x61.npz')
		...
		if len(feature_rows_temp) > 0:
			...
			if k == 0:
				for i in rand_ind:
					if (feature_rows_temp[i] - window_size_x > 0) and (feature_rows_temp[i] + window_size_x < image_size_x): 
						if (feature_cols_temp[i] - window_size_y > 0) and (feature_cols_temp[i] + window_size_y < image_size_y):
							feature_rows += [feature_rows_temp[i]]*15
							feature_cols += [feature_cols_temp[i]]*15
							feature_batch += [direc]*15
							feature_label += [k]*15
			...
			if k == 1:
				for i in rand_ind:
					if (feature_rows_temp[i] - window_size_x > 0) and (feature_rows_temp[i] + window_size_x < image_size_x): 
						if (feature_cols_temp[i] - window_size_y > 0) and (feature_cols_temp[i] + window_size_y < image_size_y):
							feature_rows += [feature_rows_temp[i]]*2
							feature_cols += [feature_cols_temp[i]]*2
							feature_batch += [direc]*2
							feature_label += [k]*2
...
print np.bincount(feature_batch)
print np.sum(feature_label == 0), np.sum(feature_label == 1), np.sum(feature_label == 2)
